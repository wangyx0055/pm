...
public class ProjectServiceImpl implements ProjectService{
	...
	public boolean addProject(User creator, Project project, List<User> members)
		...
		for (User member : members) {
			...
			ProjectMember pm = new ProjectMember(id, Constant.ROLE_MEMBER, Constant.IS_NOT_ACCESS);
			projectMembers.add(pm);
		}
		ProjectMember projectMember = new ProjectMember(new ProjectMemberId(project, creator), Constant.ROLE_MEMBER, Constant.IS_NOT_ACCESS);
		...
		if(users.size() == pms.size()) {
			for(int i = 0; i < pms.size(); i++) {
				for (int j = 0; j < users.size(); j++) {
					if(pms.get(i).getId().getUser().getId().equals(users.get(j).getId())) {
						// 成员不变
						if(pms.get(i).getId().getUser().getEmail().equals(users.get(j).getEmail()))
							continue;
						else if(this.hasUser(users.get(j)))	// 成员变化
							pms.get(i).getId().setUser(userDao.findByEmail(users.get(j).getEmail()));
						else {
							userDao.saveUser(new User(users.get(j).getEmail(), users.get(j).getPassword(), users.get(j).getName(), DateFormatUtil.DateToString(new Date()), Constant.STATUS_IS_USABLE, null, null));
							pms.get(i).getId().setUser(userDao.findByEmail(users.get(j).getEmail()));
						}
					}
				}
				...
				for (int j = 0; j < pms.size(); j++) {
					if(StringUtils.isBlank(users.get(i).getId())){
						...
						for (int k = 0; k < pms.size(); k++) {
							if(users.get(i).getEmail().equals(pms.get(k).getId().getUser().getEmail())) {
								f = false;break;
							}
						}
						if(f) {
							if(this.hasUser(users.get(i)))
								pms.add(new ProjectMember(new ProjectMemberId(project, userDao.findByEmail(users.get(i).getEmail())),Constant.ROLE_MEMBER,Constant.IS_NOT_ACCESS));
							else {
								userDao.saveUser(new User(users.get(i).getEmail(), users.get(i).getPassword(), users.get(i).getName(), DateFormatUtil.DateToString(new Date()), Constant.STATUS_IS_NOT_USABLE, null, null));
								User user = userDao.findByEmail(users.get(i).getEmail());
								ProjectMemberId pmId = new ProjectMemberId(project, user);
								ProjectMember pm = new ProjectMember(pmId, Constant.ROLE_MEMBER, Constant.IS_NOT_ACCESS);
								pms.add(pm);
							}
						}
					} else if(pms.get(j).getId().getUser().getId().equals(users.get(i).getId())) {
						// 成员不变
						if(pms.get(j).getId().getUser().getEmail().equals(users.get(i).getEmail()))
							continue;
