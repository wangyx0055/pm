...
public class StringUtil {
	 ...
	 * @param offset
	 *            偏移值，通常设置为0
	 * @return indexs 所有匹配索引
	 */
	public static List<Integer> getAllMatchIndexes(String match, String data,
			int offset) {
		List<Integer> indexs = new ArrayList<Integer>();
		for (int startIndex = 0; startIndex < data.length(); startIndex++) {
			startIndex = data.indexOf(match, startIndex);
			if (startIndex < 0)
				break;
			indexs.add(startIndex + offset);
		}
		return indexs;
	}
	...
	public static String underscoreName(String name) {
		...
		if (name != null && name.length() > 0) {
			...
			for (int i = 1; i < name.length(); i++) {
				...
				if (isLetters(s) && s.equals(s.toUpperCase())) {
					result.append("_");
	 ...
	 * @return 返回追加之后的字符串
	 */
	public static String appendString(String current, int num, String append) {
		if (null == append)
			return current;
		if (num < 0)
